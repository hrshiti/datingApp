    ...
    <div className="w-full">
            ...
            {photos.map((photo, index) => (
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="absolute top-2 right-2 bg-[#FF91A4] text-white rounded-full p-1.5 sm:p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  <X className="w-3 h-3 sm:w-4 sm:h-4" />
                </motion.button>
                {index === 0 && (
                  <div className="absolute bottom-2 left-2 bg-[#FF91A4] text-white text-xs px-2 py-1 rounded-lg font-medium">
                    Main
                  </div>
                )}
              </motion.div>
      ...
      {photos.length < maxPhotos && (
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center text-center">
            ...
            >
              {photos.length === 0 ? (
                <ImageIcon className="w-12 h-12 sm:w-16 sm:h-16 text-[#FF91A4] mb-3 sm:mb-4" />
              ) : (
                <Upload className="w-8 h-8 sm:w-12 sm:h-12 text-[#FF91A4] mb-3 sm:mb-4" />
              )}
            </motion.div>
            
            <p className="text-sm sm:text-base font-medium text-[#212121] mb-1">
              {photos.length === 0 
                ? 'Upload your photos' 
      ...
      {photos.length > 0 && (
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-xs sm:text-sm text-[#757575] mt-3 text-center"
        >
